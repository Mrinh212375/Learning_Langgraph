...
def call_to_content_supervisor(state: SupervisorState) -> SupervisorState:

    article = state["text_to_change"]
    content_result = contentflow.invoke({"article": article})
    return { "text_to_change": "Summary: " + content_result["summary"] + "\nSentiment: " + content_result["sentiment"] }
...
def call_to_distribution_supervisor(state: SupervisorState) -> SupervisorState:

    summary = state["text_to_change"].split("\n")[0].replace("Summary: ", "")
    sentiment = state["text_to_change"].split("\n")[1].replace("Sentiment: ", "")
    distribution_result = distributionflow.invoke({"summary": summary, "sentiment": sentiment})
    return { "text_to_change": "Tweet: " + distribution_result["tweet"] + "\nLinkedIn: " + distribution_result["linkedin"] }


Supervisorgraph = StateGraph(SupervisorState)
Supervisorgraph.add_node("content_supervisor", call_to_content_supervisor)
Supervisorgraph.add_node("distribution_supervisor", call_to_distribution_supervisor)
...
if __name__ == "__main__":
    ...
    result = workflow.invoke({"text_to_change": article})
    # print(f"Tweet: {result['tweet']}")
    # print(f"LinkedIn: {result['linkedin']}")
    # print(f"Summary: {result['summary']}")
    # print(f"Sentiment: {result['sentiment']}")
    print(f"\n\n result: {result['text_to_change']}")
